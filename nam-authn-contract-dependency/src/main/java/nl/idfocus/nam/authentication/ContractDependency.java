...
/**
 * This authentication class can check if a user has previously been authenticated (i.e. if we are in an authenticated session) and redirect to a configured
 * contract id if this is not the case.
 */
public class ContractDependency extends LocalAuthenticationClass {

    private static final Logger logger = LogFormatter.getConsoleLogger(ContractDependency.class.getName());
    ...
    private String buildAuthenticationURL(String returl) {
        ...
        sb.append(m_Request.getRequestURI()).append("?").append("id=").append(contractId).append("&target=").append(returl);
        return sb.toString();
    }
